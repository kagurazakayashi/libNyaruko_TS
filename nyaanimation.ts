...
export default class NyaAnimation extends NyaLib {
  ...
  /**
   * 執行自定義動畫序列
   * @param {number | HTMLElement | Record<string, string>} list 動畫序列二維陣列
   * 每個設定物件包括 [網頁元素,動畫字典,動畫時長] （示例見 README.md ）
   * @param {Function} callback 動畫完成之後的回撥
   * @param {Function} step 每個步驟動畫完成之後的回撥
   */
  static animateList(
    list: (number | HTMLElement | Record<string, string>)[][],
    callback?: () => void,
    step?: (obj?: HTMLElement, stepi?: number, total?: number) => void
  ): void {
    const stepi = 0;
   ...
   */
  static animateCallback(
    obj: HTMLElement,
    rmTransition = true,
    callback?: (obj?: HTMLElement) => void
  ): void {
    const cssAniListen = "Transitionend"; // Animationend
    const listenPfx = ["webkit", "moz", "MS", "o", ""];
    for (let i = 0; i < listenPfx.length; i++) {
      const pfx = listenPfx[i];
      listenPfx[i] =
        pfx.length == 0 ? cssAniListen.toLowerCase() : pfx + cssAniListen;
    }
    if (callback) {
      const animationend = () => {
        callback(obj);

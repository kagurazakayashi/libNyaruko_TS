...
export default class NyaCompress {
  ...
  static baseDic(charsOrLevel: string | number = 64, valBase = 2): string[][] {
    const chars = isNaN(Number(charsOrLevel))
      ? (charsOrLevel as string)
      : NyaStrings.chars(charsOrLevel as number);
    ...
    for (let i = 0; i < chunkLen; i++) {
      ...
      for (const charBin of charBinDic) {
        if (char == charBin[0]) {
          ...
          const diffLen: number =
            i == chunkLen - 1
              ? endChunksLen - binStr.length
              : maxBinLen - binStr.length;

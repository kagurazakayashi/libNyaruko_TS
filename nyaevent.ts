...
export default class NyaEvent {
     ...
     */
    static listEventListenerTypes(eventListener?: NyaEventListener | null, eventElement?: HTMLElement | null): string[] {
        if (!eventElement) {
        ...
        const nowEventStr: string | null = eventElement.getAttribute(this.nyaEventStr);
        if (nowEventStr == null) {
            return [];
        }
     ...
     */
    static removeEventListener(eventListener: NyaEventListener, options?: boolean | AddEventListenerOptions): boolean {
        if (eventListener.type != null && eventListener.listener != null && eventListener.domObj != null) {
            ...
            const nowEventStr: string | null = eventListener.domObj.getAttribute(this.nyaEventStr);
            let nowEvents: string[] = [];
            if (nowEventStr) {
                nowEvents = nowEventStr.split(',');

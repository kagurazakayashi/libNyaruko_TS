...
export class NyaTemplateElement extends NyaLib {
  code = "";
  isHTML = false;
  status = 0; // -1错误 0未完成 1完成
  nullAlert = "TEMP NULL";

  loadTo(
    dom: HTMLElement,
    isAppend = false,
    config: NyaTemplateConfig = NyaTemplateConfigDefault
  ) {
  ...
  codeByID(
    ...
    return this.isHTML
      ? NyaTemplate.loadTemplateHtml(this.code, config)
      : NyaTemplate.loadTemplateCss(this.code, config);
  }
}
...
export default class NyaTemplate extends NyaLib {
  ...
  static loadTemplate(
    templateURL: string,
    dom?: HTMLElement,
    callback?: (templateElement: NyaTemplateElement) => void,
    config: NyaTemplateConfig = NyaTemplateConfigDefault
  ): NyaTemplateElement {
  ...
  static loadTemplateHtml(
    ...
    if (config.replaceList) {
      for (const replaceKV of config.replaceList) {
        ...
        templateHTML = config.replaceAll
          ? NyaStrings.replaceAll(templateHTML, replaceK, replaceV)
          : templateHTML.replace(replaceK, replaceV);
      }
    }
    return templateHTML;
  }
